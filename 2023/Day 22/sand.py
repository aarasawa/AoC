...
Each brick is made up of a single straight line of cubes, and the Elves were even careful to choose a time for the 
snapshot that had all of the free-falling bricks at integer positions above the ground, so the whole snapshot is aligned 
...
A line like 0,0,1~0,0,10 represents a ten-cube brick which is oriented vertically. One end of the brick is the cube located 
...
removing it, no other bricks would fall further directly downward. Don't actually disintegrate any bricks - just determine 
what would happen if, for each brick, only that brick were disintegrated. Bricks can be disintegrated even if they're 
...
reference: https://www.reddit.com/r/adventofcode/comments/18o7014/comment/kegg6n0
3D block stuff, im super confused
"""
import re
...
stack = sorted([[*map(int, re.findall(r'\d+', l))]
                for l in open('input')], key = lambda b : b[2])
