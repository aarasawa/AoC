...
that the original card 10 won: cards 11, 12, 13, 14, and 15. This process repeats until none of the copies 
...
card_total = 0
cards = {x + 1: 1 for x in range(212)}
...
for row in dat:
    ...
    wins = set(int(x) for x in wl[2:])
    check = set(int(x) for x in chk)

    # find sets of winning numbers and take out empty sets
    shared = wins & check 
    if not shared:
        continue
    print(shared)
